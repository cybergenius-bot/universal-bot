# SmartPro 24/7 — ANTI‑ECHO HOTFIX#6 (Greeting, Full /menu, Stories trigger, /version)
# UX:
# - /start: приветствие (без принудительных панелей).
# - /menu: ПОЛНОЕ inline‑меню (Помощь • Оплатить • Рефералы • Профиль • Сменить язык • Режим ответа • Дай голосом • Показать расшифровку • Скрыть).
# - Голос: без повтора; авто‑TTS на войс отключён.
# - /version команда и HTTP GET /version возвращают версию сборки.
# - Текст «сторис про …»/«напиши сторис про …» — сразу выдаёт кадры Stories.
...
VERSION = "ANTI‑ECHO v2025‑09‑02 HOTFIX#6"
BOT_USERNAME = ""
...
def t_ui(lang: str = "ru"):
    data = {
        ...
        "en": dict(
            hello="Hi! I’m SmartPro 24/7 — universal assistant for text, voice and media.\nOpen actions via /menu.",
            ...
            tts_caption="Voiced",
            pay_stub="Payments soon: Stripe Checkout (USD $10 / $20 / $50).",
        ...
        "he": dict(
            hello="שלום! אני SmartPro 24/7. פתחו /menu כדי לראות פעולות זמינות.",
            menu_title="תפריט פעולות",
            help="עזרה", pay="לתשלום", refs="הפניות",
            profile="פרופיל", lang="החלפת שפה", mode="מצב תגובה",
            ...
            tts_caption="הוקרא",
            pay_stub="תשלומים בקרוב: Stripe (USD $10 / $20 / $50).",
...
def want_stories(txt: str) -> bool:
    t = (txt or "").lower()
    return any(k in t for k in ["сторис", "stories", "story", "истории", "мпе"])

def extract_topic(txt: str) -> str:
    t = (txt or "").strip()
    m = re.search(r"(?:про|about)\s+(.+)$", t, re.IGNORECASE)
    return (m.group(1).strip() if m else "") or "море"

def build_stories(topic: str, mode: str, lang: str="ru") -> str:
    n = {"short":4,"expanded":6,"deep":7}.get(mode,"expanded")
    topic = topic.capitalize()
    frames = [
        f"Кадр 1 — Хук: {topic}. Один вопрос: готовы?",
        f"Кадр 2 — Контраст: почему {topic} — это ресурс, а не побег.",
        "Кадр 3 — Практика: 3 мини‑ритуала: дыхание 4‑7‑8, 5‑мин скан тела, взгляд на горизонт.",
        "Кадр 4 — Ошибка: «берём всё». Возьмите лишь 20% — остальное лишнее.",
        "Кадр 5 — Чек‑лист: вода, SPF 50, таймер тени 45 мин, обувь, пакет для гаджетов.",
        "Кадр 6 — Микро‑история: как 15 минут дают +2 часа фокуса.",
        "Кадр 7 — CTA: «Сделай план выходных под тему — {topic}».",
    ]
    text = "Готовые Stories:\n" + "\n".join("— "+frames[i] for i in range(min(n,len(frames))))
    return text + "\n\nПодсказка: скажи «Сделай обложку/заголовок» — сгенерирую варианты."
...
# ---------- Текст: сторис‑триггер + автоязык + ловушка «скрытого эха» ----------
@dp.message(F.text)
async def on_text(m: Message):
    ...
    if want_stories(txt):
        ...
        await m.answer(build_stories(topic, mode, Lc))
        return
