...
"""
Конвертация входного аудио/видео в WAV 16kHz mono через ffmpeg.
Синхронно (CPU). Вызывайте через asyncio.to_thread().
"""
proc = subprocess.run(
    [
...
"""
Синхронный вызов OpenAI Whisper (audio.transcriptions.create).
Возвращает только текст (response_format='text').
"""
...
await update.message.reply_text("Покупка скоро. В работе PayPal и Telegram Stars.")
...
if not update.message or not update.message.text:
    return
await update.message.reply_text(f"Вы написали: {update.message.text.strip()}")
---------- Голос ----------
...
try:
    ...
    logger.exception("Audio STT error: %s", e)
    await update.message.reply_text("Не удалось распознать аудио.")
---------- Видеокружок ----------
...
try:
    ...
    # Возвращаем 200, чтобы Telegram не делал бессмысленные ретраи
