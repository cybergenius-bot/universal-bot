...
Приложение FastAPI
app = FastAPI(title="Telegram Bot", version="1.2.0", lifespan=lifespan)
@app.get("/")
async def root():
...
try:
    ...
    if update:
        ...
        return {"ok": True}
    # 200, чтобы Telegram не делал бессмысленные ретраи
    return JSONResponse({"ok": False, "error": "invalid update"}, status_code=200)
except Exception:
    # 200, чтобы Telegram не долбил ретраями при неожиданных ошибках
